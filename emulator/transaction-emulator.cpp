...
td::Result<std::unique_ptr<TransactionEmulator::EmulationResult>> TransactionEmulator::emulate_transaction(
    ...
    if (rand_seed_) {
      compute_phase_cfg.block_rand_seed = rand_seed_.unwrap();
    }
    ...
    return std::make_unique<TransactionEmulator::EmulationSuccess>(std::move(trans_root), std::move(account), std::move(trans->compute_phase->vm_log), std::move(trans->compute_phase->actions), std::move(trans->compute_phase->c7));
}
