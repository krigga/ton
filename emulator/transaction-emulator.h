...
public:
  ...
  struct EmulationSuccess: EmulationResult {
    ...
    EmulationSuccess(td::Ref<vm::Cell> transaction_, block::Account account_, std::string vm_log_, td::Ref<vm::Cell> actions_, td::Ref<vm::Tuple> c7_) :
      EmulationResult(vm_log_), transaction(transaction_), account(account_) , actions(actions_), c7(c7_)
    {}
  };
